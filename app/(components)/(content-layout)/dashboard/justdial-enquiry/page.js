...
const STATUSES = ["New", "Contacted", "Qualified", "Closed", "Not Interested"];
...
export default function JustDialEnquiryPage() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("jd_enquiry_leads");
  ...
  function saveLeads(newLeads) {
    setLeads(newLeads);
    try {
      localStorage.setItem("jd_enquiry_leads", JSON.stringify(newLeads));
    } catch (_) {}
  }

  async function importLeads() {
    if (!url.trim()) {
      setError("Enter a JustDial business URL.");
      return;
    }
    setError("");
    setLoading(true);
    try {
      const res = await axios.post("/api/justdial_enquiry", { url });
      ...
      if (newLeads.length === 0) {
        setError("No leads found at this URL. The page may require login or have no public enquiries.");
      } else {
  ...
  function exportCSV() {
    ...
    const headers = ["name", "phone", "message", "date", "category", "status", "notes"];
    const csv = [
      headers.join(","),
      ...leads.map((r) =>
        headers
    ...
    <div>
      ...
      <div className="box mb-5">
        <div className="box-header">
          <h5 className="box-title flex items-center gap-2">
            <i className="ri-customer-service-2-line text-indigo-600" />
            JustDial Enquiry CRM
            <span className="text-xs bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded font-semibold ml-1">
              v18 NEW
            </span>
          </h5>
        </div>
        <div className="box-body">
          <p className="text-sm text-gray-500 mb-4">
            Import enquiry leads from a JustDial business listing and manage
            them in this CRM. Data is stored locally in your browser.
          </p>
          <div className="flex gap-3 items-end flex-wrap">
            <div className="flex-1 min-w-[240px]">
              <label className="block text-sm font-medium text-gray-600 mb-1">
                JustDial Business URL
              </label>
              <input
      ...
      {leads.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 mb-5">
          {STATUSES.map((s) => (
            <div key={s} className="box mb-0 cursor-pointer" onClick={() => setStatusFilter(statusFilter === s ? "All" : s)}>
              <div className={`box-body py-3 rounded-lg transition-all ${statusFilter === s ? "ring-2 ring-indigo-400" : ""}`}>
          ...
          <div className="box-body p-0 overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {["#", "Name", "Phone", "Message", "Date", "Category", "Status", "Notes", ""].map(
                    (h) => (
                      <th
                        key={h}
                  ...
                  <tr key={l.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <select
                        ...
                        className={`text-xs rounded-full px-2 py-1 font-medium border-0 outline-none cursor-pointer ${STATUS_COLORS[l.status] || "bg-gray-100 text-gray-600"}`}
                      >
                        {STATUSES.map((s) => (
                          <option key={s} value={s}>
                            {s}
      ...
      {leads.length === 0 && (
        <div className="box">
          <div className="box-body text-center py-14">
            <i className="ri-customer-service-2-line text-5xl text-indigo-200" />
            <p className="text-gray-400 mt-3 text-base">No leads yet.</p>
            <p className="text-gray-400 text-sm mt-1">
              Paste a JustDial business listing URL above and click &quot;Import Leads&quot;.
            </p>
          </div>
        </div>
      )}
