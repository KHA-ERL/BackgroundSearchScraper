...
const columns = [
  ...
  { key: "price", header: "Price (MRP)" },
  { key: "discounted_price", header: "Discounted" },
...
export default function MyntraScraperPage() {
  ...
  async function run() {
    ...
    setLoading(true);
    try {
      const res = await axios.post("/api/myntra_scraper", {
        query,
        maxResults: Math.min(Number(maxResults) || 20, 50),
      });
      setRows(res.data.data || []);
    } catch (e) {
      setError(e?.response?.data?.error || "Failed to scrape Myntra.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function exportCSV() {
    ...
    a.download = "myntra-products.csv";
    a.click();
  }
            ...
            <div className="box">
              <div className="box-body">
                <div className="flex items-center gap-3">
                  <span className="w-10 h-10 rounded-full bg-pink-50 flex items-center justify-center">
                    <i className={`${s.icon} text-pink-500 text-xl`} />
                  </span>
                  <div>
      ...
      <div className="box mb-5">
        <div className="box-header">
          <h5 className="box-title flex items-center gap-2">
            <i className="ri-shirt-line text-pink-500" /> Myntra Scraper
            <span className="text-xs bg-pink-100 text-pink-600 px-2 py-0.5 rounded font-semibold ml-1">
              v15 NEW
            </span>
          </h5>
        </div>
        <div className="box-body">
          <div className="grid grid-cols-12 gap-4 mb-4">
            <div className="col-span-12 md:col-span-9">
              ...
              <input
                ...
                placeholder="e.g. Men's casual shirts, Women's kurta, Running shoes"
          ...
          <div className="flex gap-3 flex-wrap">
            <button
              onClick={run}
              disabled={loading}
              className="ti-btn ti-btn-outline border border-pink-500 bg-pink-500 text-white hover:bg-pink-600 flex items-center gap-2 disabled:opacity-60"
            >
              {loading ? (
                <>
                ...
                <>
                  <i className="ri-search-line" /> Search Myntra
                </>
              )}
            </button>
            {rows.length > 0 && (
        ...
        <div className="box-body p-0 overflow-x-auto">
          {rows.length > 0 ? (
                    ...
                    {columns.map((c) => (
                      ...
                      >
                        {c.key === "url" && row[c.key] ? (
                          ...
                          <span className="bg-pink-100 text-pink-700 px-2 py-0.5 rounded text-xs font-medium">
                            {row[c.key]}
                          </span>
            ...
            <p className="text-center text-gray-400 py-10">
              {loading ? "Scraping Myntra..." : "No products to display"}
            </p>
          )}
        </div>
      </div>
